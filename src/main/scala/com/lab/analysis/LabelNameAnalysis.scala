...
object LabelNameAnalysis {
  ...
  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setMaster("local[*]").setAppName("label").set("spark.driver.host", "localhost")
    val spark = SparkSession.builder().config(conf).getOrCreate()
    val sc: SparkContext = spark.sparkContext

    val rdd = sc.hadoopFile("file://"+INPUT_PATH, classOf[TextInputFormat],
    ...
    //将RDD映射到rowRDD
    val rowRDD: RDD[Row] = labelRdd.map(p=>{Row(p)})
    ...
    lableData.write.mode("append").jdbc("jdbc:mysql://localhost:3306/goods", "label",prop)
    sc.stop()
  }
}
